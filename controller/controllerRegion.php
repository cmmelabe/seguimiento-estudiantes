...
class ControlRegion
{
    ...
    {
        $con = new Conexion();
        $rs = mysqli_query($con->getConnection(), "SELECT id, nombre, numero, romano, activo FROM region;");
        if ($rs) {
        ...
        mysqli_close($con->getConnection());
        return $this->lista;
    }
    ...
    public function addLista($_lista)
    {
        $listaSalida = array();
        $con = new Conexion();
        //se recorre la lista que llega por parametro
        $this->lista = $this->getLista();
        $id = count($this->lista) + 1;
        echo $id;
        foreach ($_lista as $_objeto) {
            //asigna id
            //crea sql
            $sql = "INSERT INTO region (id, nombre, numero, romano, activo) VALUES ($id, '" . $_objeto['nombre'] . "', " . $_objeto['numero'] . ", '" . $_objeto['romano'] . "', true);";
            // colocar try catch, y que solo retorne false en el catch
            try {
                if ($con->getConnection()->query($sql) === TRUE) {
                    array_push($listaSalida, $this->getLast());
                    $id += 1;
                }
            } catch (\Throwable $th) {
                mysqli_close($con->getConnection());
    ...
    public function deleteById($_id)
    {
        ...
        $registro = $this->getById($_id);
        if (isset($registro)) {
            $con = new Conexion();
            $sql = "DELETE FROM area WHERE id = " . $_id;
            if ($con->getConnection()->query($sql) === TRUE) {
                mysqli_close($con->getConnection());
                return true;
            }
    ...
    public function disableById($_id)
    {
        $con = new Conexion();


        $sql = "UPDATE area SET activo = 0 WHERE id = " . $_id;
    ...
    public function enableById($_id)
    {
        $con = new Conexion();

        $sql = "UPDATE area SET activo = 1 WHERE id = " . $_id;
    ...
    public function updateByObj($_obj)
    {
        $con = new Conexion();

        $sql = "UPDATE area SET nombre = '" . $_obj->getNombre() . "' WHERE id = " . $_obj->getId();
    ...
    public function updateAPIByObj($_obj)
    {
        //valido si existe antes de pegarle a la base de datos
        $existe = $this->getById($_obj->getId());
        if (isset($existe)) {
            //hay cambios para aplicar en la base de datos?
            $hayCambios = strcmp($_obj->getNombre(), $existe['nombre']);
            if ($hayCambios) {
                $con = new Conexion();
                $sql = "UPDATE area SET nombre = '" . $_obj->getNombre() . "' WHERE id = " . $_obj->getId();
                try {
                    if ($con->getConnection()->query($sql) === TRUE) {
                        mysqli_close($con->getConnection());
                        //necesito que me devuelva el objeto con los datos actualizados
                        $objetoDesactualizado = $this->getById($_obj->getId()); //desactualizado, nos interesa solo el activo (lo que no se actualizo ni paso por parametro)
                        $objetoRetorno = array(
                            "id" => $_obj->getId(),
                            "nombre" => $_obj->getNombre(),
                            "activo" => $objetoDesactualizado['estado']
                        );
